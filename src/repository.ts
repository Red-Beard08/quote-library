...
export class QuoteRepository {
  ...
  async getTopics(): Promise<TopicRecord[]> {
    ...
    const records = await Promise.all(files.map(async file => { try { return await this.readTopic(file); } catch (error) { console.warn(`Quote Library could not read topic ${file.path}.`, error); return null; } }));
  ...
  async writeQuote(record: QuoteRecord, cleanupKnownLegacy = true): Promise<void> {
    ...
    await this.app.vault.process(file, content => {
      const parsed = parseNote(content); const managed = this.quoteBody(record); const preserved = cleanupKnownLegacy && knownLegacyBody(parsed.body) ? removeKnownLegacyBody(parsed.body) : parsed.body; let body = replaceManagedBlock(preserved, QUOTE_START, QUOTE_END, managed);
