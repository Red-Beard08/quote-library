...
import { dailyIndex, dateKey, topicKey } from "./utils";
...
export class QuoteLibraryDashboard extends ItemView {
  ...
  private overview(root: HTMLElement, quotes: QuoteRecord[], duplicateCount: number, unreadablePaths: string[]): void {
    ...
    const incompleteQuotes = quotes.filter(incomplete); if (incompleteQuotes.length) this.quoteList(root, incompleteQuotes.slice(0, 8), "Needs attention");
...
function incomplete(quote: QuoteRecord): boolean { return !quote.source || !quote.topics.length || quote.legacy; }
async function copyQuote(quote: QuoteRecord): Promise<void> { try { await navigator.clipboard.writeText(`“${quote.text}” — ${quote.author}${quote.source ? `, ${quote.source}` : ""}`); new Notice("Quote copied."); } catch { new Notice("The quote could not be copied on this device."); } }
