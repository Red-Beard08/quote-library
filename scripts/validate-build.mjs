...
if (manifest.version !== "1.1.1" || pkg.version !== manifest.version || versions[manifest.version] !== manifest.minAppVersion) failures.push("Release versions are inconsistent.");
...
for (const marker of ["quote-library-dashboard", "add-quote", "Preview migration", "Modernize filenames", "Restore latest backup", "quote-library:quote:start", "Quote of the day", "Review duplicates"]) if (!main.includes(marker)) failures.push(`main.js is missing ${marker}.`);
